// SennaChat — clone do SeuZeChat com mudanças cirúrgicas pro Senna:
//   - useSenna (em vez de useSeuZe)
//   - Gate PRO mantido (funileiro/automotivo é profissional pago)
//   - Avatar próprio (urso com boné e jaleco SENNA, pistola HVLP)
//   - Saudação e copy ajustados pra automotivo/funilaria
//   - Cor de destaque: vermelho-Senna #e10600 (F1)
//
// Quando vier 5ª persona, refatorar todos em AiPersonaChat parametrizado.
...
const SENNA_AVATAR: AvatarConfig = {
  src: '/img/senna.webp',
  alt: 'Senna',
  typingLabel: 'Senna está digitando',
};

export function SennaChat() {
  ...
  const fireGreeting = useCallback(() => {
    ...
    void speakText(
      'Fala, mestre. Sou o Senna. Em qual serviço você tá hoje — pintura, lanternagem, polimento?',
    );
  }, [speakText]);
  ...
  if (!user) {
    return (
      <div className="text-center py-12 px-4 rounded-2xl bg-white border border-[color:var(--color-border)]">
        ...
        <h2 className="font-semibold mb-2">Entre pra conversar com o Senna</h2>
        <p className="text-sm text-[color:var(--color-muted)] mb-4">
          O mestre da pintura tá te esperando — faça login pra começar.
        </p>
        <Link
          href="/login"
  ...
  // Gate PRO: Senna é assistente de funileiro/preparador profissional.
  if (!canSeeProFeature(policyUser)) {
    return (
      <div className="text-center py-12 px-4 rounded-2xl bg-white border border-[color:var(--color-border)]">
        <div className="text-5xl mb-3" aria-hidden="true">🚗</div>
        <h2 className="font-bold mb-2 text-[color:var(--color-ink)]">
          Conversar com o Senna é PRO
        </h2>
        <p className="text-sm text-[color:var(--color-muted)] mb-4 max-w-md mx-auto">
          Tira dúvida sobre PU 2K, primer, verniz, lanternagem, polimento
          e preço médio do mercado.
        </p>
        <Link
          href="/pro"
          className="inline-block px-5 py-2 bg-[color:var(--color-p1)] text-white rounded-xl font-semibold"
    ...
    >
        ...
        <div className="flex items-center gap-3">
          {isVoiceSupported ? (
            <button
              ...
              style={{
                color: conversationMode ? '#e10600' : 'var(--color-muted)',
              }}
            >
              {conversationMode ? '🟢' : '⚪'}
            </button>
      ...
      >
        {messages.length === 0 && !isSending ? (
          <div
            ...
            style={{
              background: 'linear-gradient(135deg, rgba(225,6,0,.05), rgba(255,107,53,.04))',
              borderRadius: 14,
          ...
          >
            ...
            >
              🚗 Conta o serviço — pintura, polimento, lanternagem
            </p>
            <p
              className="text-sm"
            ...
            >
              Manjo de PU 2K, primer, verniz, polimento, ceramic coating e preço médio do mercado.
            </p>
          </div>
        ) : null}
        ...
        {isSending || isTranscribing ? <TypingIndicator avatar={SENNA_AVATAR} /> : null}
      ...
      >
        <textarea
          ...
          placeholder="Pergunte ao Senna…"
          rows={1}
          className="flex-1 min-w-0 resize-none px-3 py-2 text-sm border border-[color:var(--color-border)] rounded-xl focus:outline-none focus:ring-2 max-h-32"
          style={{ ['--tw-ring-color' as never]: '#e10600' }}
          aria-label="Mensagem para o Senna"
        ...
        <button
          ...
          className="shrink-0 px-4 py-2 text-white rounded-xl text-sm font-bold disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-opacity"
                    ...
                    return (
                      <li
                        ...
                        style={{
                          ...
                          background: isActive ? 'rgba(225,6,0,.06)' : '#fff',
                        }}
                      >
                        <div className="flex items-center gap-2" style={{ padding: '12px 16px' }}>
                          ...
                          >
                            <div
                              ...
                              style={{
                                fontSize: 13,
                                color: isActive ? '#e10600' : 'var(--color-ink)',
                              }}
                            >

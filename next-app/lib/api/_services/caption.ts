// lib/api/_services/caption.ts — port de
// `functions/api/_services/caption.js`. Gera legenda + hashtags pra post.
...
export async function generateCaption(args: {
  ...
  if (!image || typeof image === 'string') {
    throw new ServiceError('campo "image" obrigatório', 400);
  }
  ...
  try {
    ...
    throw new ServiceError(
      'falha lendo imagem: ' + (e instanceof Error ? e.message : String(e)),
      400
    );
  }
        ...
        messages: [
          { role: 'system', content: SYSTEM_PROMPT },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'Gere a legenda e as hashtags para esta foto do meu portfólio de pintura/decoração.',
              },
  ...
  if (!caption && hashtags.length === 0) {
    throw new ServiceError('IA devolveu vazio', 502);
  }
  return { caption, hashtags };
}

function sanitizeCaption(v: unknown): string {

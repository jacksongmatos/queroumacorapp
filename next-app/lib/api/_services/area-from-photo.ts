// lib/api/_services/area-from-photo.ts — port de
// `functions/api/_services/area-from-photo.js`. Estima m² pintável via vision.
...
export async function estimateAreaFromPhoto(args: {
  image: File | Blob | FormDataEntryValue | null;
}): Promise<{ area_m2: number; justification: string }> {
  ...
  try {
        ...
        messages: [
          { role: 'system', content: SYSTEM_PROMPT },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'Estime a metragem pintável visível nesta foto, em m². Lembre-se: seja conservador e exclua janelas/portas.',
              },
    ...
    let parsed: { area_m2?: unknown; justification?: unknown };
    try {
    ...
    return {
      ...
      justification:
        justification ||
        'Estimativa visual aproximada — revise antes de comprar.',
    };
  } catch (e) {

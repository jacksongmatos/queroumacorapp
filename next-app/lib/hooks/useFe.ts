// useFe — clone do useSeuZe com 3 mudanças cirúrgicas:
//   1. Storage prefix `fe:` (sessões separadas do Seu Zé)
//   2. Chat endpoint /api/fe (PRO, mesma gate do Seu Zé)
//   3. TTS reusa /api/tts com voice 'echo' (masculino jovem, vibe street)
//
...
// (em prod) intocado durante o lançamento do Fê. Quando vier 5ª persona,
...
const AUTO_SPEAK_KEY = 'fe:autoSpeak';
const CONVERSATION_MODE_KEY = 'fe:conversationMode';
const SESSIONS_KEY_PREFIX = 'fe:sessions:';
const ACTIVE_SESSION_KEY_PREFIX = 'fe:activeSession:';
const CHAT_ENDPOINT = '/api/fe';
// Fê é PRO — reusa /api/tts do Seu Zé (gateProAI), só troca a voice.
...
export function useFe(): UseFeResult {
  const { user } = useAuth();
